...
try:
    df = pd.read_csv("/app/fpl_2020_2024_historical_data.csv")
    print("📦 Loaded local historical CSV")
    for _, row in df.iterrows():
        try:
            ...
            print(f"❌ Insert error (Player {row.get('player_id')}):", e)
            conn.rollback()
    conn.commit()
except Exception as e:
    print("❌ Failed to load local CSV:", e)
